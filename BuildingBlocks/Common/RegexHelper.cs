...
{
    private static readonly Regex EmailPrimaryRegex = 
        new Regex(@"(@)(.+)$", RegexOptions.Compiled, 200.Milliseconds());
        
    private static readonly Regex EmailSecondaryRegex = 
        new Regex(
            @"^(?("")("".+?(?<!\\)""@)|(([0-9a-z]((\.(?!\.))|[-!#\$%&'\*\+/=\?\^`\{\}\|~\w])*)(?<=[0-9a-z])@))" +
    ...
    public bool IsValidEmail(string input)
    {
        ...
        try
        {
            replaced = EmailPrimaryRegex.Replace(input, DomainMapper);
        ...
        try
        {
            return EmailSecondaryRegex.IsMatch(replaced);
        } catch (RegexMatchTimeoutException)
        {
            return false;
        }
