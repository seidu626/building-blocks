...
public abstract class TreeNodeBase<T> where T : TreeNodeBase<T>
{
    ...
    private static readonly ContextState<Dictionary<string, object>> ContextState = new ContextState<Dictionary<string, object>>("TreeNodeBase.ThreadMetadata", HttpContextAccessor);
    ...
    public void SetThreadMetadata(string key, object value)
    {
        ...
        var state = ContextState.GetState();
        if (state == null)
        {
    ...
    {
        Guard.AgainstNull(newParent, nameof(newParent));
    ...
    {
        Guard.AgainstNull(refNode, nameof(refNode));

        var refParent = refNode._parent;
        if (refParent == null)
        {
            throw Errors.Argument("refNode", "The reference node cannot be a root node and must be attached to the tree.");
        }
